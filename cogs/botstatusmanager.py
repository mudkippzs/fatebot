...
class botstatusmanager(commands.Cog):
    ...
    async def _status_rotate(self):  # pylint: disable=unused-argument; _status_rotate takes no arguments but is called with one due to use as an event handler (see main)
        ...
        await self.bot.wait_until_ready()  # wait until bot has logged in and initialised before running code
        while not self.bot.is_closed():  # loop forever until bot closes
            ...
            for status in status_list:  # iterate through each item in the list, setting the bots status message to each item sequentially
                await self.bot.change_presence(activity=discord.Activity(name=status[0],type=status[1]))  # change the bots presence by setting its activity attribute to a discord Game object containing our new status message
                await asyncio.sleep(600)  # sleep for 5 seconds before moving on to next iteration of loop
