            ...
            if description:
                if description.lower() != inv_item[2].lower():
                    ...
                    global_inventory[str(
                        user)][idx][2] = global_inventory[str(user)][idx][2]
...
class InventoryManager(commands.Cog):
    ...
    async def admininventory(self, ctx, user: discord.Member = None, command: str = "check", item: str = None, quantity: int = 1, description: str = "No description"):
        ...
        if "add" in command:
            inventory = add_to_user_inventory(
                user.id, item, quantity, description)
            ...
            search_item, search_item_quantity, search_item_description = search_in_user_inventory(
                user.id, item)
            if search_item == 0:
            ...
            await ctx.send(f"```{ctx.message.author} {command_verb(command)} {user}'s inventory...```")
            random_item, random_item_quantity, random_item_description = get_random_item_from_inventory(
                user.id)
            if random_item != 0:
    ...
    async def inventory(self, ctx, command: str = "check", item: str = None, quantity: int = 1, description: str = "No description"):
        ...
        if "add" in command:
            inventory = add_to_user_inventory(
                ctx.message.author.id, item, quantity, description)
            await ctx.send(f"```{ctx.message.author} {command_verb(command)} their inventory: `{item}` ```")
            await ctx.send(inventory)

        elif "remove" in command:
            inventory = remove_from_user_inventory(
                ctx.message.author.id, item, quantity)
