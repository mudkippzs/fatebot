...
def attr_to_dots(value=1, epic=False, ability=False):
    ...
    if ability:
        ...
        attr_row = f"".rjust(value, bs) + (ws * (10 - value)) + cstar # 11th epic level is god-tier 'star' 
    else:
        attr_row = f"".rjust(value, bc) + (wc * (10 - value))
    return attr_row

def yards_to_kph(yards):
    return round((yards * 3600) / 1760, 2)

def yards_to_mph(yards):
    return round(((yards * 3600) / 1760) / 1.609, 2)

def yards_to_mps(yards):
    return round((yards + 1760) / 3600, 2)
...
class Character(commands.Cog):
        ...
        for root, dirs, files in os.walk(character_dir):  # pylint: disable=unused-variable
            for file in files:
                if file.startswith("pc_") and file.endswith(".py"):  # only python script is allowed to be imported.
                    ...
                    module_name = os.path.basename(player_file)[:-3]  # remove ".py" from the end of the filename
                    spec = importlib.util.spec_from_file_location(module_name, player_file)  # pylint: disable=no-member
                    module = importlib.util.module_from_spec(spec)  # pylint: disable=no-member
                    spec.loader.exec_module(module)  # pylint: disable=no-member
    ...
    async def load_characters(self, ctx = None):
        ...
        most_recent_characters_exports = [k for k in sorted(os.listdir(directory), key=lambda f: os.path.getmtime(os.path.join(directory, f)), reverse=True) if k.endswith(".py") == False][0:len(player_ids)]
        import_roster = []
    ...
    def refill_player_sheet_charges(self, target: discord.Member = None, initial = False):
        ...
        if target:
            player_sheet = [(idx, pc) for idx, pc in enumerate(self.character_roster) if pc["player_id"] == target.id][0]
    ...
    def recalculate_player_sheet(self, initial = False):
        ...
        for idx, pc in enumerate(self.character_roster):
            ...
            player_sheet["soak"]["bludgeon"] = player_sheet["attributes"]["sta"] + calculate_epic(player_sheet["epic_attributes"]["epic_sta"])
            player_sheet["soak"]["lethal"] = round(player_sheet["soak"]["bludgeon"] / 2)
            player_sheet["soak"]["aggrevated"] = round(player_sheet["epic_attributes"]["epic_sta"] / 2)
            
            # Combat
            player_sheet["combat"]["dodge"][0] = (player_sheet["attributes"]["dex"] + player_sheet["abilities"]["athletics"] + player_sheet["legend"]) + calculate_epic(player_sheet["epic_attributes"]["epic_dex"]) / 2
            pc_brawl = player_sheet["abilities"]["brawl"]
            pc_melee = player_sheet["abilities"]["melee"]

            if pc_melee >= pc_brawl:
                player_sheet["combat"]["parry"][0] = (player_sheet["attributes"]["dex"] + pc_melee) + calculate_epic(player_sheet["epic_attributes"]["epic_dex"]) / 2
            else:    
                player_sheet["combat"]["parry"][0] = (player_sheet["attributes"]["dex"] + pc_brawl) + calculate_epic(player_sheet["epic_attributes"]["epic_dex"]) / 2
            
            player_sheet["join_battle"] = player_sheet["attributes"]["wits"] + player_sheet["abilities"]["awareness"] + calculate_epic(player_sheet["epic_attributes"]["epic_wits"])

            # Movement
            player_sheet["movement"]["move"] = player_sheet["attributes"]["dex"] + calculate_epic(player_sheet["epic_attributes"]["epic_dex"])
            ...
            player_sheet["movement"]["jump"]["vertical"] = player_sheet["attributes"]["stre"] + calculate_epic(player_sheet["epic_attributes"]["epic_stre"]) + player_sheet["abilities"]["athletics"]
            player_sheet["movement"]["jump"]["horizontal"] = player_sheet["movement"]["jump"]["horizontal"] * 2
    ...
    async def stats(self, ctx, private = True, target: discord.Member = None, chained = False):
        ...
        player_sheet = [pc for pc in self.character_roster if pc["player_id"] == player_key]
        player_sheet = player_sheet[0]
        if chained == False:
            embed = discord.Embed(title=f"Attributes :: {player_sheet['name']} ```Player: {ctx.author.display_name}```")
        else:
            embed = discord.Embed(title=f"Character Sheet :: {player_sheet['name']} ```Player: {ctx.author.display_name}```")
        
        sorted_attributes = sort_attr_list(player_sheet["attributes"], False)
        sorted_epic_attributes = sort_attr_list(player_sheet["epic_attributes"], True)
        for attribute in sorted_attributes:
            attr_list = []
            attr_list.append(attr_to_dots(sorted_attributes[attribute], False)) # Standard Attrs
            attr_list.append(attr_to_dots(sorted_epic_attributes[f"epic_{attribute}"], True)) # Epic Attrs
            attr_string = "\n".join(attr_list)
    ...
    async def myboons(self, ctx, private = True, target: discord.Member = None, chained = False):
        ...
        embed.add_field(name="\u200b", value=f"```{', '.join(boon_list)}```", inline=True)
    ...
    async def myknacks(self, ctx, private = True, target: discord.Member = None, chained = False):
        ...
        embed.add_field(name="\u200b", value=f"```{', '.join(knack_list)}```", inline=True)
    ...
    async def abilities(self, ctx, private = True, target: discord.Member = None, chained = False):
        ...
        for ability in sorted_abilities:
            ability_list = []
            ability_list.append(attr_to_dots(sorted_abilities[ability], False, True)) # Standard Attrs
            ability_str = "\n".join(ability_list)
            embed.add_field(name=ability.title(), value=ability_str, inline=True)
    ...
    async def combat(self, ctx, private = True, target: discord.Member = None, chained = False):
        ...
        join_battle = player_sheet["attributes"]["wits"] + player_sheet["abilities"]["awareness"]

        embed = discord.Embed(title="")
        if chained == False:
            embed.title = f"Combat Stats :: {player_sheet['name']} ```Player: {ctx.author.display_name}```"

        embed.add_field(name="\u200b",value=f"**Dodge DV**  `{dodge_dv:^5}`", inline=True)
        embed.add_field(name="\u200b",value=f"**Parry DV**  `{parry_dv:^5}`", inline=True)
        embed.add_field(name="\u200b",value=f"**Join Battle**  `{join_battle:^5}`", inline=True)
        embed.add_field(name="\u200b",value="**Soak**", inline=False)
        embed.add_field(name="\u200b",value=f"**Bludgeon**  `{soak_b:^5}`", inline=True)
        embed.add_field(name="\u200b",value=f"**Lethal**  `{soak_l:^5}`", inline=True)
        embed.add_field(name="\u200b",value=f"**Aggrevated**  `{soak_a:^5}`", inline=True)
        embed.add_field(name="\u200b",value="**Armor**", inline=False)
        embed.add_field(name="\u200b",value=f"**Bludgeon**  `{armor_b:^5}`", inline=True)
        embed.add_field(name="\u200b",value=f"**Lethal**  `{armor_l:^5}`", inline=True)
        embed.add_field(name="\u200b",value=f"**Aggrevated**  `{armor_a:^5}`", inline=True)
    ...
    async def movement(self, ctx, private = True, target: discord.Member = None, chained = False):
        ...
        jump_v = player_sheet["movement"]["jump"]["horizontal"] 
        jump_h = player_sheet["movement"]["jump"]["vertical"]
        ...
        embed.add_field(name="\u200b", value=f"```{yards_to_kph(move)} KmH\n{yards_to_mps(move)} m/s\n{yards_to_mph(move)} MpH```")
        embed.add_field(name="\u200b", value=f"```{yards_to_kph(dash)} KmH\n{yards_to_mps(dash)} m/s\n{yards_to_mph(dash)} MpH```")
        embed.add_field(name="\u200b", value=f"```{yards_to_kph(climb)} KmH\n{yards_to_mps(climb)} m/s\n{yards_to_mph(climb)} MpH```")
        embed.add_field(name="Jump - Vertical", value=f"```{jump_v} ({yards_to_meters(jump_v)} meters)```")
        embed.add_field(name="Jump - Horizontal", value=f"```{jump_h} ({yards_to_meters(jump_h)} meters)```")
