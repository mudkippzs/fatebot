...
def parse_scion(scion, soup):
    ...
    name = scion.find(
        'strong', text='Name').next_sibling.text.strip().replace(": ", "")

    aka = scion.find('strong', text='AKA').next_sibling.strip(
    ).replace(": ", "").split(",")

    description = "\n\n".join([d.text for d in soup.find_all('p') if len(
        d.text) > 20][1:]).split("Associated")[0].replace(": ", "")

    epic_attributes = [p.strip() for p in scion.find(
        'strong', text='Associated Powers').next_sibling.strip().replace(": ", "").split(",") if "Epic" in p]
    purviews = [p.strip() for p in scion.find('strong', text='Associated Powers').next_sibling.strip(
    ).replace(": ", "").split(",") if "Epic" not in p]

    abilities = scion.find('strong', text='Abilities').next_sibling.strip(
    ).replace(": ", "").split(",")

    rivals = scion.find('strong', text='Rivals').next_sibling.replace(
        ": ", "").replace(";", ",").strip().split(",")
