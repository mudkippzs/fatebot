...
def get_random_first_names_by_country(country, gender):
	...
	f_name_path = pathlib.Path(f"{name_path}/{country}/first_names_{gender.lower()}.json")
	names = None
	#print(f_name_path)
...
def generate_name_files_by_country_all_genders():
	...
	for gender in ["Male", "Female"]:
		for country in country_list:
			first_names = nd.get_top_names(n=500, gender=gender, country_alpha2=country)

			pathlib.Path(f"{name_path}/{country}/").mkdir(parents=True, exist_ok=True)
	...
	for country in country_list:
		surnames = nd.get_top_names(n=500, country_alpha2=country, use_first_names=False)
		l_name_path = pathlib.Path(f"{name_path}/{country}/last_names.json")
